...
mlow = 10**massbins[0][0]/1e14
mhigh = 10**massbins[0][-1]/1e14



plot(outerrange, [scatter[0] for scatter in scatters], c='b', linestyle='--', linewidth=3.0, label='%1.1f x10^14' % mlow)
plot(outerrange, [scatter[-1] for scatter in scatters], c='k', linewidth = 3.0, label='%1.1f x10^14' % mhigh)
#plot(outerrange, [scatter[8] for scatter in scatters], c='k', linewidth = 3.0, label='%1.1f x10^14' % mid500)
...
#
axis([0.5, 2.7, 0.0, 0.55])

ax = gca()
ax.set_xticks(map(float, '0.5 1.0 1.5 2.0 2.5'.split()))
ax.set_xticklabels('0.5 1.0 1.5 2.0 2.5'.split(), fontsize=16)

ax.set_yticks(np.arange(0.0, 0.60, 0.1))
ax.set_yticklabels(['%1.2f' % x for x in np.arange(0.0, 0.60, 0.1)], fontsize=16)



xlabel('Outer Fit Radius [Mpc]', fontsize=18)
ylabel(r'Intrinsic Scatter from Nearby Structure', fontsize=18)
legend(title=r'Cluster $M_{200}$')
...
savefig('scatter_v_fitradius.png')
savefig('scatter_v_fitradius.eps')
savefig('scatter_v_fitradius.pdf')
