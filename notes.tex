...
Since we are still looking at the same probability distribution, $\^ \beta$ that maximizes $P(\beta|\vec y)$ where $P(\veta) = \mathrm{constant}$, we have $\^\beta = f(\^\alpha)$ for $\alpha$ that maximized $P(\vec y|\alpha)$. 
...
Therefore, the MAP for $\beta$ is not $\^ \beta$.
...
For Joerg:
  - His paper is feeding right into a scaling relations code. That code has P(M_lens | M_true) in it. 
